...
class Image_GD_Driver extends Image_Driver {
	...
	public function resize($properties)
	{
		...
		if ($properties['master'] === EImage::AUTO)
		{
			// Change an automatic master dim to the correct type
			$properties['master'] = (($width / $properties['width']) > ($height / $properties['height'])) ? EImage::WIDTH : EImage::HEIGHT;
		}

		if ($properties['master'] === EImage::COVER) {
			$properties['master'] = (($width / $properties['width']) < ($height / $properties['height'])) ? EImage::WIDTH : EImage::HEIGHT;
		}

		if (empty($properties['height']) OR $properties['master'] === EImage::WIDTH)
		{
			// Recalculate the height based on the width
			$properties['height'] = round($height * $properties['width'] / $width);
		}

		if (empty($properties['width']) OR $properties['master'] === EImage::HEIGHT)
		{
