...
Class NSprite extends CApplicationComponent
{
	 ...
	 */
	public function registerSpriteCss($containerWidth = 0, $containerHeight = 0){
		if (!empty($containerWidth) && !empty($containerHeight)) {
	...
	private function _generateImage(){
		...
		if($total['width'] > 0 && $total['height'] > 0){
			...
			foreach($this->_images as $i=>$image){
				...
				if (!empty($this->_containerHeight) && !empty($this->_containerWidth)) {
					if ($image['width'] > $this->_containerWidth || $image['height'] > $this->_containerHeight) {
						$ratio = max($image['width'] / $this->_containerWidth, $image['height'] / $this->_containerHeight);
					} else {
						$ratio = 1;
					}
					imagecopyresampled($sprite, $img, ($total['width'] - ($image['width']/$ratio))/2, $top+(($this->_containerHeight - ($image['height']/$ratio))/2), 0, 0, $image['width']/$ratio, $image['height']/$ratio, $image['width'], $image['height']);
				} else {
					imagecopy($sprite, $img, ($total['width'] - $image['width'])/2, $top, 0, 0,  $image['width'], $image['height']);
				}
				$top += $this->_containerHeight;
			}
	 ...
	 * @return void
		...
		$total = $this->_totalSize();
		$top = $total['height'];
		$css = '.'.$this->cssSpriteClass.'{background-image:url(sprite.png); margin: 2.5px;}'."\n";
		// for 16x16 icons
		...
		if (!empty($this->_containerWidth) || !empty($this->_containerHeight)) {
			$top = 0;
			foreach($this->_images as $i=>$image) {
				$css .= '.'.$image['name'].'{';
				$css .= 'background-position: 0px '.$top.'px;';
				$css .= 'width:'.$this->_containerWidth.'px;';
				$css .= 'height:'.$this->_containerHeight.'px;';
				$css .= '}'."\n";
				$top += $this->_containerHeight;
			}
		} else {
	 ...
	 */
		...
		if (!empty($this->_containerHeight)) {
			$arr['height'] = count($this->_images)*$this->_containerHeight;
		}
		if (!empty($this->_containerWidth) && $arr['width'] > $this->_containerWidth) {
			$arr['width'] = $this->_containerWidth;
		}
